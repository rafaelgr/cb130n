...
app.get('/customers/:code', function(request, response){
  ...
  /*
  // LAB 8 OPTIONAL CHALLENGE - Lookup by attribute of embedded document
  // + Verify Customer model embeds related Country documents
  // + Verify Customer model support indexed lookup by countryCode of embedded Country
  // Lookup Customers by value in a related, embedded Country document

  models.Customer.findByCountryCode(countryCode, function(err, customers){
    if (err) {
      response.status(404).json({'error': err});
    } else if (customers.length == 0) {
      response.status(200).json({'status': 'No document matched ' + countryCode});
...
app.post('/customers', jsonParser, function(request, response) {
  ...
  /*  
  // RELATION BY DOCUMENT REFERENCE
  // + Update Customer model to reference a Country doc as country attribute

  // Convert the country field of the posted Customer document to a reference

  customerData.billingAddress.country = models.Country.ref(countryCode);
...
app.put('/customers/:id', jsonParser, function(request, response){
  ...
  models.Customer.getById(customerId, function(err, customer){
    ...
    customer.firstName = customerDoc.firstName;
    customer.lastName = customerDoc.lastName;
    customer.email = customerDoc.email;
    ...
    // Remove this Customer model
     ...
     customer.remove(function(err){
      ...
      if(err) { 
        ...
        // response.json(customer);
        response.json({'status': 'Document deleted: ' + customerId });
      };
